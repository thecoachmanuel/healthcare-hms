...
const LabScientistDashboardPage = async () => {
  ...
  const [catalogApprovedCount, catalogPendingCount, pendingRequests, completedToday] = await Promise.all([
    db.services.count({ where: { category: "LAB_TEST" as any, approved: true, ...(staff?.lab_unit_id ? { lab_unit_id: staff.lab_unit_id } : {}) } }),
    db.services.count({ where: { category: "LAB_TEST" as any, approved: false, ...(staff?.lab_unit_id ? { lab_unit_id: staff.lab_unit_id } : {}) } }),
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold">Lab Scientist</h1>
        ...
        <div className="flex gap-2">
          ...
          {!staff?.lab_unit_id && (
            <Link href="/lab_scientist/unit" className="px-3 py-1.5 rounded-md border text-sm">Set My Unit</Link>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card><CardHeader className="pb-2"><CardDescription>Catalog (Pending)</CardDescription><CardTitle className="text-2xl">{catalogPendingCount}</CardTitle></CardHeader></Card>
