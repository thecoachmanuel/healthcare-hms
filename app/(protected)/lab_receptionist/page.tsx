  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold">Lab Receptionist</h1>
          <p className="text-sm text-gray-600">Unit: <span className="font-medium">{unit?.name ?? "Not set"}</span></p>
        </div>
        <div className="flex gap-2">
          {!staff?.lab_unit_id && (
            <Link href="/lab_receptionist/unit" className="px-3 py-1.5 rounded-md border text-sm">Set My Unit</Link>
          )}
          ...
          <Link href="/notifications" className="px-3 py-1.5 rounded-md bg-emerald-600 text-white hover:bg-emerald-700 text-sm">Notifications</Link>
        </div>
      </div>
