"use client";

import {
  createDoctorSpecialization,
  updateDoctorSpecialization,
} from "@/app/actions/catalog";
import { CustomInput } from "@/components/custom-input";
import { Button } from "@/components/ui/button";
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
  DialogTrigger,
} from "@/components/ui/dialog";
import { Form } from "@/components/ui/form";
import { zodResolver } from "@hookform/resolvers/zod";
import { Pencil, Plus } from "lucide-react";
import { useRouter } from "next/navigation";
import React, { useState } from "react";
import { useForm } from "react-hook-form";
import { toast } from "sonner";
import { z } from "zod";

const SpecSchema = z.object({
  name: z.string().min(2),
  department: z.string().optional(),
  active: z.boolean().optional(),
});

export function AddSpecialization() {
  const [loading, setLoading] = useState(false);
  const router = useRouter();
  const form = useForm<z.infer<typeof SpecSchema>>({
    resolver: zodResolver(SpecSchema),
    defaultValues: { name: "", department: "" },
  });

  const onSubmit = async (values: z.infer<typeof SpecSchema>) => {
    try {
      setLoading(true);
      const res = await createDoctorSpecialization({
        name: values.name,
        department: values.department,
      });
      if (res.success) {
        toast.success(res.msg);
        form.reset({ name: "", department: "" });
        router.refresh();
      } else toast.error(res.msg);
    } catch (e: any) {
      toast.error(e?.message ?? "Failed to create specialization");
    } finally {
      setLoading(false);
    }
  };

  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button size="sm" className="text-sm font-normal">
          <Plus size={18} className="text-gray-500" /> Add Specialization
        </Button>
      </DialogTrigger>
      <DialogContent className="max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Add Specialization</DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <CustomInput type="input" control={form.control} name="name" label="Name" placeholder="" />
            <CustomInput type="input" control={form.control} name="department" label="Department" placeholder="" />
            <Button type="submit" disabled={loading} className="bg-blue-600 w-full">
              {loading ? "Saving..." : "Save"}
            </Button>
          </form>
        </Form>
      </DialogContent>
    </Dialog>
  );
}

export function EditSpecialization({
  id,
  name,
  department,
  active,
}: {
  id: number;
  name: string;
  department: string | null;
  active: boolean;
}) {
  const [loading, setLoading] = useState(false);
  const router = useRouter();
  const form = useForm<z.infer<typeof SpecSchema>>({
    resolver: zodResolver(SpecSchema),
    defaultValues: { name, department: department ?? "", active },
  });

  const onSubmit = async (values: z.infer<typeof SpecSchema>) => {
    try {
      setLoading(true);
      const res = await updateDoctorSpecialization({
        id,
        name: values.name,
        department: values.department,
        active: values.active ?? true,
      });
      if (res.success) {
        toast.success(res.msg);
        router.refresh();
      } else toast.error(res.msg);
    } catch (e: any) {
      toast.error(e?.message ?? "Failed to update specialization");
    } finally {
      setLoading(false);
    }
  };

  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button size="sm" variant="outline">
          <Pencil size={16} className="text-gray-500" />
          Edit
        </Button>
      </DialogTrigger>
      <DialogContent className="max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit Specialization</DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <CustomInput type="input" control={form.control} name="name" label="Name" placeholder="" />
            <CustomInput type="input" control={form.control} name="department" label="Department" placeholder="" />
            <CustomInput type="checkbox" control={form.control} name="active" label="Active" placeholder="Active" />
            <Button type="submit" disabled={loading} className="bg-blue-600 w-full">
              {loading ? "Saving..." : "Save"}
            </Button>
          </form>
        </Form>
      </DialogContent>
    </Dialog>
  );
}

