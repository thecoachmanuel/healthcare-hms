...
export const NewPatient = ({ data, type }: DataProps) => {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data }) => {
      if (!data.user || !data.user.email) return;
      const meta =
        (data.user.user_metadata as { first_name?: string; last_name?: string }) ??
        {};
      setAuthUser({
        id: data.user.id,
  ...
  const onSubmit: SubmitHandler<z.infer<typeof PatientUpdateSchema>> = async (
    ...
    const payload = { ...values, img };

    const res =
      type === "create"
        ? await createNewPatient(payload, userId!)
        : await updatePatient(payload, userId!);
